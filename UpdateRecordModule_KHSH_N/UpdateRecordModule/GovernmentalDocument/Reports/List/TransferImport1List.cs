...
namespace UpdateRecordModule_KHSH_N.GovernmentalDocument.Reports.List
{
    ...
    /// 轉學生入學名冊
    /// </summary>
    public class TransferImport1List : ReportBuilder
    {
        ...
        protected override void Build(System.Xml.XmlElement source, string location)
        {
            ...
            wb.Open(new MemoryStream(Properties.Resources.TransferImport1Template));
            // 解析 XML            
            XElement elmRoot = XElement.Parse(source.OuterXml);
            Worksheet wst = wb.Worksheets["轉學生入學書面"];
            ...
            if (elmRoot != null)
            {
                ...
                foreach (string gS in tmpElmList.Keys)
                {
                    foreach (KeyValuePair<string, List<XElement>> data in tmpElmList[gS])
                    {
                        ...
                        wst.Cells[rowIdx - 2, 7].PutValue("頁數：" + PageCount);
                        foreach (XElement elmE in data.Value)
                        {
                            if (sp_count > PageMaxStudent)
                            {
                                ...
                                wst.Cells[rowIdx, 0].PutValue(ScYear + gS + "年級");
                                wst.Cells[rowIdx, 3].PutValue(data.Key + " " + utility.GetXMLAttributeStr(elmRoot, "類別"));
                                wst.Cells[rowIdx, 7].PutValue("頁數：" + PageCount);
                                rowIdx++;
                                wst.Cells.CreateRange(rowIdx, 1, false).Copy(R_Title);
                                rowIdx++;
                            }
                            ...
                            // 學校代號,學校名稱
                            wst.Cells[rowIdx, 4].PutValue(utility.GetXMLAttributeStr(elmE, "學校代碼") + "\n" + utility.GetXMLAttributeStr(elmE, "學校名稱"));
                            // 科碼,科別
                            wst.Cells[rowIdx, 5].PutValue(utility.GetXMLAttributeStr(elmE, "科別代碼") + "\n" + utility.GetXMLAttributeStr(elmE, "科別"));
                            // 年級
                            wst.Cells[rowIdx, 6].PutValue(utility.GetXMLAttributeStr(elmE, "年級"));
                            // 核准日期+核准文號
                            wst.Cells[rowIdx, 7].PutValue(utility.DateParse1(utility.GetXMLAttributeStr(elmE, "備查日期")) + "\n" + utility.GetXMLAttributeStr(elmE, "備查文號"));
                            // 日期,原因(異動日期,原因)
                            wst.Cells[rowIdx, 8].PutValue(utility.DateParse1(utility.GetXMLAttributeStr(elmE, "異動日期")) + "\n" + utility.GetXMLAttributeStr(elmE, "原因及事項"));
                            // 備註
                            wst.Cells[rowIdx, 9].PutValue(utility.GetXMLAttributeStr(elmE, "備註"));
                            sp_count++;
                            rowIdx++;
                        }
