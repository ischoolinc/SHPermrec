...
{
    public partial class ConfigForm_放 : BaseForm
    {
        ...
        public ConfigForm_放(string defaultTemplate1, byte[] buffer1, string defaultTemplate2, byte[] buffer2, string defaultTemplate3, byte[] buffer3)
        {
            InitializeComponent();
        ...
        private void buttonX1_Click(object sender, EventArgs e)
        {
            ...
            if (config == null)
            {
                config = new XmlDocument().CreateElement("放棄學籍通知單範本");
            }
            //範本選擇設定
        ...
        private void linkLabel2_LinkClicked(object sender, LinkLabelLinkClickedEventArgs e)
        {
            SaveFileDialog sfd = new SaveFileDialog();
            sfd.Title = "另存新檔";
            sfd.FileName = "放棄學籍通知單_合併欄位總表.docx";
            sfd.Filter = "Word檔案 (*.docx)|*.docx|所有檔案 (*.*)|*.*";
            if (sfd.ShowDialog() == DialogResult.OK)
            {
                try
