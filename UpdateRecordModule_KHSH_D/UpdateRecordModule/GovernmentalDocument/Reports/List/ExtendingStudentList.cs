...
namespace UpdateRecordModule_KHSH_D.GovernmentalDocument.Reports.List
{
    ...
    public class ExtendingStudentList : ReportBuilder
    {
        ...
        protected override void Build(System.Xml.XmlElement source, string location)
        {
            ...
            wb.Open(new MemoryStream(Properties.Resources.ExtendingStudentTemplate));
            // 解析 XML            
            XElement elmRoot = XElement.Parse(source.OuterXml);
            Worksheet wst = wb.Worksheets["延修生書面"];
            ...
            if (elmRoot != null)
            {
                ...
                int rowIdx = 0, PageMaxStudent = 11;
                ...
                foreach (string gS in tmpElmList.Keys)
                {
                    foreach (KeyValuePair<string, List<XElement>> data in tmpElmList[gS])
                    {
                        ...
                        foreach (XElement elmE in data.Value)
                        {
                            ...
                            // 班級
                            wst.Cells[rowIdx, 0].PutValue("");
                            // 學號
                            wst.Cells[rowIdx, 1].PutValue(utility.GetXMLAttributeStr(elmE, "學號"));
                            // 姓名
                            wst.Cells[rowIdx, 2].PutValue(utility.GetXMLAttributeStr(elmE, "姓名"));
                            // 入學學年度
                            wst.Cells[rowIdx, 3].PutValue("");
                            // 應畢業學年度
                            wst.Cells[rowIdx, 4].PutValue(utility.GetXMLAttributeStr(elmE, "應畢業學年度"));

                            // 電話
                            wst.Cells[rowIdx, 5].PutValue("");

                            // 登記延修日期
                            wst.Cells[rowIdx, 6].PutValue("");
