...
namespace UpdateRecordModule_SH_D.GovernmentalDocument.Reports.List
{
    public class UtilXml
    {
        ...
        public static XmlElement SortByGraduateCertificateNumber(XmlElement source)
        {
            XmlElement retVal = null;
            XElement elmRoot = XElement.Parse(source.OuterXml);
            ...
            foreach (XElement d1Elm in dataElms)
            {
                ...
                try
                {
                    ...
                    upRecList = (from elm in d1Elm.Elements("異動紀錄") orderby elm.Attribute("畢業證書字號").Value select elm).ToList();
