...
{
    public partial class TransferingStudentUpdateRecordListModifyingCoverForm2021 : FISCA.Presentation.Controls.BaseForm
    {

        StudUpdateRecBatchRec _BRec;

        List<TransferStudUpdateRecCoverRec> coverDataList = new List<TransferStudUpdateRecCoverRec>();
        List<TransferStudUpdateRecCoverRec> coverDataListA = new List<TransferStudUpdateRecCoverRec>();

        //2018/3/6 穎驊新增 提供使用者 可以自行調整 轉入生名冊封面的資料
        public TransferingStudentUpdateRecordListModifyingCoverForm2021(StudUpdateRecBatchRec BRec)
        {
            InitializeComponent();
            ...
            foreach (XmlNode list in source.SelectNodes("清單"))
            {
                ...
                foreach (XmlElement st in list.SelectNodes("異動名冊封面"))
                {
                   ...
                   // string classType = st.SelectSingleNode("@班別").InnerText;
                    ...
                    //轉入學生數
                    row_data.Add(transferStudentCount);           
                    //現有學生數
        ...
        private void btnSave_Click(object sender, EventArgs e)
        {
            ...
            {
                TransferStudUpdateRecCoverRec coverData = new TransferStudUpdateRecCoverRec();
                ...
                coverData.TransferStudentCount = "" + dr.Cells["轉入學生數"].Value;
        ...
        public XElement ConvertStudUpdateRecDocToXML(List<BL.StudUpdateRecDoc> updateRecList)
        {
            ...
            foreach (KeyValuePair<string, List<BL.StudUpdateRecDoc>> val in data)
            {
                ...
                foreach (TransferStudUpdateRecCoverRec coverRec in coverDataList)
                {
                    ...
                    if (coverRec.grYear == grYear && coverRec.Department == DeptName && coverRec.ClassType == classType)
                    {
                        ...
                        elmGrDeptCover.SetAttributeValue("轉入學生數", coverRec.TransferStudentCount);                        
            ...
            {
                Boolean find = false;
                foreach (TransferStudUpdateRecCoverRec coverRecA in coverDataListA)
                {
