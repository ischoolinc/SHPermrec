...
namespace UpdateRecordModule_SH_D
{
    public class PermRecLogProcess
    {
        ...
        /// </summary>
        /// <returns></returns>
        public bool GetDataHasChange()
        {
            bool retVal = false;
            foreach (KeyValuePair<string, string> str in _BeforeData)
            {
                if (_AfterData.ContainsKey(str.Key))
                {
                    if (_AfterData[str.Key] != str.Value)
                    {
                        retVal = true;
                        break;
                    }
            ...
            if (retVal == false)
                if (_BeforeData.Count != _AfterData.Count)
                    retVal = true;
