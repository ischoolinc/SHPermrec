...
namespace UpdateRecordModule_SH_N.ImportExport
{
    public partial class ImportStudentV2 : FISCA.Presentation.Controls.BaseForm, SmartSchool.API.PlugIn.Import.ImportWizard
    {
        ...
        {            
            ...
            { 
                ...
                case "休學":
                    ...
                    break;
                case "延修":
                    _StudStatus = K12.Data.StudentRecord.StudentStatus.延修;
                    break;
                case "畢業或離校":
        ...
        private void _BKWValidate_DoWork(object sender, DoWorkEventArgs e)
        {
            ...
            if (importFields.ContainsKey("學生系統編號"))
            {
                ...
                {                    
                    ...
                    if (id != "")
                    {
                        ...
                        #region 驗明正身
                        SHStudentRecord stu = SHStudent.SelectByID(id);
                        if (stu != null)
                        {
            ...
            {
                #region 用學號驗證資料
                Dictionary<string, List<SHStudentRecord>> studentNumberStudents = new Dictionary<string, List<SHStudentRecord>>();
                #region 整理學號對應學生清單(如索引欄不試系統編號時用)
                foreach (SHStudentRecord stu in SHStudent.SelectAll())
                {
                    if (stu.Status == _StudStatus )
                    {
                        if (!studentNumberStudents.ContainsKey(stu.StudentNumber))
                            studentNumberStudents.Add(stu.StudentNumber, new List<SHStudentRecord>(new SHStudentRecord[] { stu }));
                        else
                        ...
                        if (studentNumberStudents.ContainsKey(num))
                        {
                            if (studentNumberStudents[num].Count > 1)
                            {
                                ...
                                foreach (string validateKey in new string[] { "班級", "科別", "座號", "姓名" })
                                {
                                    ...
                                    {
                                        err = false;
                                        foreach (SHStudentRecord var in studentNumberStudents[num])
                                        {
                                            bool pass = true;
                            ...
                            if (studentNumberStudents[num].Count == 1)
                            {
                                SHStudentRecord var = studentNumberStudents[num][0];
                                bool pass = true;
                                //if (importFields.ContainsKey("班級") && GetTrimText("" + wb.Worksheets[0].Cells[i, importFields["班級"]].StringValue) != (var.Class != null ? var.Class.Name : ""))
                                //{
                                //    if (studentNumberStudents[num].Count == 1)
